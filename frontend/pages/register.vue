<template>
  <div class="bg-slate-900/50 backdrop-blur-xl border border-white/10 p-8 rounded-2xl shadow-2xl">
    <div class="text-center mb-8">
      <h1 class="text-3xl font-bold text-white mb-2">Регистрация</h1>
      <p class="text-slate-400">Присоединяйтесь к сообществу ITSTEP</p>
    </div>
    ...
    <div class="mt-6 text-center text-sm">
      <span class="text-slate-400">Уже есть аккаунт? </span>
      <NuxtLink to="/login" class="text-blue-400 hover:text-blue-300 font-medium">Войти</NuxtLink>
    </div>

    <div v-if="success" class="mt-4 p-4 bg-green-500/10 border border-green-500/20 rounded-lg text-green-400 text-sm text-center">
...
const handleRegister = async () => {
  ...
  try {
    ...
    if (e.code === 'auth/email-already-in-use') {
      ...
      msg = 'Пароль слишком слабый. Минимум 6 символов.'
    } else if (e.code === 'auth/operation-not-allowed') {
      msg = 'Регистрация по email временно отключена.'
    }
    error.value = msg
  } finally {
    loading.value = false
